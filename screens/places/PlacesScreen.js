...
const PlacesScreen = () => {
  ...
  const filters = [
    {
      id: "0",
      filter: "Costo:Menor a Mayor",
    },
    {
      id: "1",
      filter: "Costo:Mayor a Menor",
    },
  ];
  ...
  return (
    <View>
      <Pressable style={styles.headerPlaces}>
        <Pressable onPress={() => setModalVisibile(!modalVisibile)}
          style={styles.sortPressable}>
          <Octicons name="arrow-switch" size={22} color="gray" />
          <Text style={styles.textSortIcon}>Ordenar</Text>
        </Pressable>

        <Pressable style={styles.sortPressable}>
          <Ionicons name="filter" size={22} color="gray" />
          <Text style={styles.textSortIcon}>Filtrar</Text>
        </Pressable>

        <Pressable onPress={() => navigation.navigate("Map",{
          searchResults:searchPlaces,})} style={styles.sortPressable}>
          <Ionicons name="location" size={22} color="gray" />
          <Text style={styles.textSortIcon}>Mapa</Text>
        </Pressable>
      </Pressable>

      {loading ? (
        <Text>Buscando destinos....</Text>
      ) : (
        ...
        footer={
          <ModalFooter>
            ...
            onPress={() => applyFilter(selectedFilter)}
              style={styles.footerModalPressable}>
              <Text>Aplicar</Text>
            </Pressable>
          </ModalFooter>
        }
        modalTitle={<ModalTitle title="Orden y filtro" />}
        modalAnimation={
          new SlideAnimation({
            slideFrom: "bottom",
          })
        ...
        <ModalContent style={styles.modalContent}>
          <View style={styles.viewContent}>
            <View style={styles.viewContentText}>
              <Text style={styles.viewText}>Orden</Text>
            </View>

            <View style={styles.viewHeader}>
              {filters.map((item, index) => (
                <Pressable onPress={() => setSelectedFilter(item.filter)}
                  style={styles.filterMapPressable} key={index}>
                    {selectedFilter.includes(item.filter) ? (
                        <FontAwesome name="circle" size={18} color="green" />
                    ) : (
                        <Entypo name="circle" size={18} color="black" />
                    )}
                  <Text style={styles.itemFilter}>
                    {item.filter}
                  </Text>
                </Pressable>
              ))}
